...
%  $^{\dagger}$Department of Automatic Control \qquad\qquad\qquad $^{\star}$ Department of Engineering Science\\
%  and System Engineering \qquad\qquad\qquad\qquad\quad University of Oxford, UK\qquad\qquad\\
%  \quad\qquad University of Sheffield, UK \qquad\qquad\qquad\qquad\quad \texttt{olga.isupova@eng.ox.ac.uk}\\
%  \texttt{dkuzin1@sheffield.ac.uk} \qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\\
%  \texttt{l.s.mihaylova@sheffield.ac.uk} \qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\\
%}
\author{
Danil Kuzin$^{\dagger}$, Olga Isupova$^{\star}$, Lyudmila Mihaylova$^{\dagger}$\\
...
\maketitle
%\begin{abstract}
%Bayesian learning allows to estimate uncertainty of predictions in neural networks by imposing the prior distributions on weights, propagating the resulting uncertainty through the layers and computing the posterior distributions of predictions. We propose a novel method of propagating the uncertainty through the soft thresholding nonlinearity. The method expands the area of Bayesian neural networks and enables the soft thresholding nonlinearity to be included in Bayesian deep neural networks for the first time. We design a Bayesian Learned Iterative Shrinkage-Thresholding network (Bayesian \textsc{lista}). We develop an efficient posterior inference algorithm based on probabilistic backpropagation. A series of experiments on sparse coding shows that the proposed framework provides both accurate predictions and sensible estimates of uncertainty in these predictions.
%\end{abstract}
...
\end{equation}
  ...
  Prior factors (\ref{eq:ws}) and (\ref{eq:gamma_eta}) from $p$ are incorporated into $q$ with the EP algorithm~\cite{hernandez2015probabilistic}, i.e. they replace the corresponding approximating factors from $q$, and then $q$ is updated to minimise the Kullback--Leibler divergence.
   ...
   \begin{figure}[!t]
  \centering
  \subfloat[\textsc{mnist} for $K = 100$]{\includegraphics[width=0.25\textwidth]{graphics/mnist/100_nmse_valid}
  \label{fig:nmse_k_100}}
  \hfil
  \subfloat[\textsc{mnist} for $K = 250$]{\includegraphics[width=0.25\textwidth]{graphics/mnist/250_nmse_valid}
