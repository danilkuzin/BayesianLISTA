...
class NumberOfLayersExperimentReultsCollector(object):
    ...
    def collect_ista_fista(self, filename):
        ...
        for rseed in range(self.random_range):
            for i, L in enumerate(self.L_array):
                saved_res = np.load('ista_fista/fista_ista_{}_L_{}_rseed_{}.npz'.format(L, rseed, filename))
    ...
    def collect_main(self, alg_name):
        ...
        for i in range(self.random_range):
            saved_res = np.load('{}/number_of_layers_measures.npz'.format(i+1, i+1))
    ...
    def plot_all(self):
        ...
        plt.plot(self.L_array, self.ista_f_meas_train, label='ista')
        plt.plot(self.L_array, self.fista_f_meas_train, label='fista')
        plt.plot(self.L_array, self.freq_f_meas_train, label='freq lista')
        plt.plot(self.L_array, self.sh_bayes_f_meas_train, label='sh bayes')
        plt.legend()
        plt.title('F measure train')
        plt.savefig('results/f_measure_train.eps', format='eps')
        plt.show()

        plt.plot(self.L_array, self.ista_f_meas_validation, label='ista')
        plt.plot(self.L_array, self.fista_f_meas_validation, label='fista')
        plt.plot(self.L_array, self.freq_f_meas_validation, label='freq lista')
        plt.plot(self.L_array, self.sh_bayes_f_meas_validation, label='sh bayes')
        plt.legend()
        plt.title('F measure validation')
        plt.savefig('results/f_measure_validation.eps', format='eps')
        plt.show()

        plt.plot(self.L_array, self.ista_loss_train, label='ista')
        plt.plot(self.L_array, self.fista_loss_train, label='fista')
        plt.plot(self.L_array, self.freq_loss_train, label='freq lista')
        plt.plot(self.L_array, self.sh_bayes_loss_train, label='sh bayes')
        plt.legend()
        plt.title('NMSE train')
        plt.savefig('results/nmse_train.eps', format='eps')
        plt.show()

        plt.plot(self.L_array, self.ista_loss_validation, label='ista')
        plt.plot(self.L_array, self.fista_loss_validation, label='fista')
        plt.plot(self.L_array, self.freq_loss_validation, label='freq lista')
        plt.plot(self.L_array, self.sh_bayes_loss_validation, label='sh bayes')
        plt.legend()
