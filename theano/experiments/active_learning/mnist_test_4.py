...
class ActiveLearningExperiments(object):
    def __init__(self, update_size, initial_lambda=0.1):
        self.L = 20
        self.learning_rate = 0.0001
        self.n_train_iter = 50
        self.update_size = update_size
        self.inital_lambda = initial_lambda
    ...
    def init_and_pretrain_lista(self):
        self.active_bayesian_lista = SingleBayesianListaHandler(D=self.D, K=self.K, L=self.L, X=self.X,
                                                                initial_lambda=self.inital_lambda)
...
if __name__=='__main__':
    ...
    np.savez('test_4_mnist_active_rseed_{}'.format(rseed), freq_validation_loss=freq_validation_loss,
