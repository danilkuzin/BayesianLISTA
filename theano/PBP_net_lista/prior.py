...
class Prior:
    ...
    def __init__(self, L, D, K, var_targets):
        ...
        for l in range(L):
            self.rnd_m_w.append(np.random.randn(D, K))
            self.m_w_hat_nat.append(np.zeros((D, K)))
            self.v_w_hat_nat.append((self.a_w - 1) / self.b_w * np.ones((D, K)))
            self.a_w_hat_nat.append(np.zeros((D, K)))
            self.b_w_hat_nat.append(np.zeros((D, K)))

            self.rnd_m_s.append(np.random.randn(D, D))
            self.m_s_hat_nat.append(np.zeros((D, D)))
            self.v_s_hat_nat.append((self.a_s - 1) / self.b_s * np.ones((D, D)))
            self.a_s_hat_nat.append(np.zeros((D, D)))
            self.b_s_hat_nat.append(np.zeros((D, D)))
    ...
    def get_initial_params(self):
        ...
        for i in range(len(self.rnd_m_w)):
            ...
            m_s.append(self.rnd_m_s[i])
            v_s.append(1.0 / self.v_s_hat_nat[i])
    ...
    def get_params(self):
        ...
        for i in range(len(self.rnd_m_w)):
            ...
            v_w.append(1.0 / self.v_w_hat_nat[ i ])
            m_s.append(self.m_s_hat_nat[i] / self.v_s_hat_nat[i])
            v_s.append(1.0 / self.v_s_hat_nat[i])
    ...
    def refine_prior(self, params):
        ...
        for i in range(len(params[ 'W_M' ])):
            for j in range(params[ 'W_M' ][ i ].shape[ 0 ]):
                for k in range(params[ 'W_M' ][ i ].shape[ 1 ]):

                    # We obtain the parameters of the cavity distribution

                    v_w_nat = 1.0 / params[ 'W_V' ][ i ][ j, k ]
                    m_w_nat = params[ 'W_M' ][ i ][ j, k ] / \
                        params[ 'W_V' ][ i ][ j, k ]
                    v_w_cav_nat = v_w_nat - self.v_w_hat_nat[ i ][ j, k ]   # (36)
                    m_w_cav_nat = m_w_nat - self.m_w_hat_nat[ i ][ j, k ]   # (36)
                    ...
                    if v_w_cav > 0 and b_w_cav > 0 and a_w_cav > 1 and \
                        ...
                        params[ 'W_M' ][ i ][ j, k ] = m_w_new
                        params[ 'W_V' ][ i ][ j, k ] = v_w_new

                        self.a_w = a_w_new
                        self.b_w = b_w_new
                        
        for i in range(len(params[ 'S_M' ])):
            for j in range(params[ 'S_M' ][ i ].shape[ 0 ]):
                for k in range(params[ 'S_M' ][ i ].shape[ 1 ]):

                    # We obtain the parameters of the cavity distribution

                    v_s_nat = 1.0 / params[ 'S_V' ][ i ][ j, k ]
                    m_s_nat = params[ 'S_M' ][ i ][ j, k ] / \
                        params[ 'S_V' ][ i ][ j, k ]
                    v_s_cav_nat = v_s_nat - self.v_s_hat_nat[ i ][ j, k ]   # (36)
                    m_s_cav_nat = m_s_nat - self.m_s_hat_nat[ i ][ j, k ]   # (36)
                    v_s_cav = 1.0 / v_s_cav_nat
                    m_s_cav = m_s_cav_nat / v_s_cav_nat
                    a_s_nat = self.a_s - 1
                    b_s_nat = -self.b_s
                    a_s_cav_nat = a_s_nat - self.a_s_hat_nat[ i ][ j, k ]
                    b_s_cav_nat = b_s_nat - self.b_s_hat_nat[ i ][ j, k ]
                    ...
                    if v_s_cav > 0 and b_s_cav > 0 and a_s_cav > 1 and \
                        ...
                        self.m_s_hat_nat[ i ][ j, k ] = m_s_new_nat - \
                            m_s_cav_nat
                        self.v_s_hat_nat[ i ][ j, k ] = v_s_new_nat - \
                            v_s_cav_nat
                        self.a_s_hat_nat[ i ][ j, k ] = a_s_new_nat - \
                            a_s_cav_nat
                        self.b_s_hat_nat[ i ][ j, k ] = b_s_new_nat - \
                            b_s_cav_nat

                        # We update the posterior approximation

                        params[ 'S_M' ][ i ][ j, k ] = m_s_new
                        params[ 'S_V' ][ i ][ j, k ] = v_s_new
