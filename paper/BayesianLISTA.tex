...
Parameters of the approximating posterior distribution are then updated with the derivates of this normalisation constant.
...
Figure~\ref{fig:unsersampling_synthetic} gives the results of predictive performance for different observation sizes $K$. The number of layers is set as $L=4$. In the previous experiment both Bayesian and classical \textsc{lista} shows similar results with this number of layers. The results of Figure~\ref{fig:unsersampling_synthetic} confirms this competitive behaviour between two \textsc{lista} networks. Baselines show similar results in terms of F measure and underperform in terms of \textsc{nmse}.

The experiments on synthetic data demonstrate that Bayesian \textsc{lista} provides competitive results in terms of predictive accuracy while additionally estimating uncertainty of predictions.
...
Figures \ref{fig:mnist_100} and \ref{fig:mnist_250} present quality on the validation set with dictionaries of size $100$ and $250$.  The experiment with $K=100$ presents ``severe'' conditions for the algorithms: the very limited size of the training dataset combined with the small dimensionality of observations. The Baysian \textsc{lista} is able to learn even under these conditions, significantly outperforming \textsc{lista} that demonstrate very poor results. Under easier condition of the second experiment with $K=250$ both \textsc{lista} networks converge to the similar results. However, the Bayesian \textsc{lista} demonstrates remarkably better convergence rate. Both baselines are unable to learn in these experiments.
