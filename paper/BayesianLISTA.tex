...
\end{equation}
where $\delta_0$ is the delta-function that represents a spike, $[\cdot]_d$ denotes the $d$-th component of a vector. In Section \ref{sec:fprop}, we show that the output of the next layer $\widehat{\boldsymbol\beta}_l$ can be approximated with a spike and slab distribution and, therefore, the output of the \textsc{lista} network $\widehat{\boldsymbol\beta}_{L}$ has the spike and slab distribution.
...
\begin{equation}
\label{eq:likelihood}
p(\mathbf{B}| \mathbf{Y}, \mathbf{W}, \mathbf{S}, \gamma, \lambda) = \prod_{n=1}^N\prod_{d=1}^D\mathcal{N}\left(\beta_d^{(n)}; \left[\widehat{\boldsymbol\beta}_{L}^{(n)}\right]_d, \gamma^{-1}\right)
\end{equation}
