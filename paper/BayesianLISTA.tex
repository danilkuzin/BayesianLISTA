...
\item we develop an efficient posterior inference algorithm for weights and outputs of neural networks with the soft thresholding nonlinearity;
...
Matrices $\mathbf{W}$, $\mathbf{S}$ are the parameters that are initialised as in the \textsc{ista} and then updated with the backpropagation algorithm. Vectors $\mathbf{c}_l$, $\mathbf{b}$ are intermediate vectors that describe forward propagation.
...
\begin{align}
&Z \approx \int \prod_{d=1}^{D} \left[\mathcal{N}\left(\beta_d | \widehat{\beta}_d, \gamma^{-1}\right) \nonumber \right.\\
&\times \left(\omega^{\widehat{\boldsymbol\beta}}_d \delta_0\left(\widehat{\beta}_d\right) + \left(1 - \omega^{\widehat{\boldsymbol\beta}}_d\right)\mathcal{N}\left(\widehat{\beta}_d | m^{\widehat{\boldsymbol\beta}}_d, v^{\widehat{\boldsymbol\beta}}_d\right)\right) \nonumber\\
&\times \left.\vphantom{\mathcal{N}(\beta_d | \widehat{\beta}_d} \text{Gam} \left(\gamma_d; \alpha^\gamma, \beta^\gamma\right)\right]\mathrm{d}\widehat{\boldsymbol\beta} \mathrm{d}\gamma  \nonumber\\
...
& + \left(1 - \omega^{\widehat{\boldsymbol\beta}}_d\right)\int \left[\mathcal{T}\left(\beta_d | \widehat{\beta}_d, \beta^\gamma / \alpha^\gamma, 2\alpha^\gamma\right)  \nonumber \right.\\
&\times \left.\left. \vphantom{\int}  \mathcal{N}\left(\widehat{\beta}_d | m^{\widehat{\boldsymbol\beta}}_d, v^{\widehat{\boldsymbol\beta}}_d\right)\right] \mathrm{d}\widehat{\boldsymbol\beta}_d\right]  \nonumber\\
& \approx \prod_{d=1}^D \left[\omega^{\widehat{\boldsymbol\beta}}_d  \mathcal{T}\left(\beta_d | 0, \beta^\gamma / \alpha^\gamma, 2\alpha^\gamma\right) \nonumber \right.\\
\label{eq:Z_approx}
& + \left. \vphantom{m^{\widehat{\boldsymbol\beta}}_d} \left(1 - \omega^{\widehat{\boldsymbol\beta}}_d\right)\mathcal{N}\left(\beta_d | m^{\widehat{\boldsymbol\beta}}_d,  \beta^\gamma / (\alpha^\gamma - 1) + v^{\widehat{\boldsymbol\beta}}\right)\right],
\end{align}
where we have approximated the Student's t density $\mathcal{T}\left(\beta_d | \widehat{\beta}_d, \beta^\gamma / \alpha^\gamma, 2\alpha^\gamma\right)$ with a Gaussian density with the same mean and variance. The Student's t density can be parametrised in different ways. In this paper the following parametrisation is used
\begin{equation}
...
Figure~\ref{fig:unsersampling_synthetic} gives the results of predictive performance for different observation sizes $K$. The number of layers is set as $L=4$. In the previous experiment Bayesian and classical \textsc{lista} show similar results with this number of layers. The results of Figure~\ref{fig:unsersampling_synthetic} confirm this competitive behaviour between two \textsc{lista} networks. Baselines show similar results in terms of F measure and underperform in terms of \textsc{nmse}.
...
\caption{Predictive performance for different numbers of iterations on the \textsc{mnist} data with the dictionary size $K = 250$}
\label{fig:mnist_250}
\end{figure}
